...
impl Message for ErrorMessage<'_> {
    ...
    fn decode(buf: &[u8]) -> Result<Self, crate::Error> {
        ciborium::from_reader(buf).map_err(|err| {
            ...
            crate::Error::new(ErrorKind::Decode, "the Error Message")
        })
    }
        ...
        W: Write,
    {
        ciborium::into_writer(self, write).map_err(|err| {
            ...
            crate::Error::new(ErrorKind::Encode, "the Error Message")
        })
    }
}
...
mod tests {
    ...
    fn error_message_roundtrip() {
        let error_msg = ErrorMessage::new(
            ErrorCode::InternalServerError.into(),
            60,
            "some error".into(),
            None,
            None,
    ...
    fn error_message_display() {
        ...
        insta::assert_snapshot!("known_code", error_msg);
        insta::assert_debug_snapshot!("known_code_debug", error_msg);
        ...
        insta::assert_snapshot!("unknown_code", error_msg);
        insta::assert_debug_snapshot!("unknown_code_debug", error_msg);
    }
    ...
    fn error_message_known_code_roundtrip() {
        ...
        for code in error_codes {
            let error_msg = ErrorMessage::new(code.into(), 60, "some error".into(), None, None);
