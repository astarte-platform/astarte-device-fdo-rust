...
impl HttpRetry {
    ...
    fn should_retry_code(resp: &reqwest::Response) -> bool {
        Self::is_actuall_internal_error(resp)
            || matches!(
                resp.status(),
    ...
    fn is_actuall_internal_error(resp: &reqwest::Response) -> bool {
        resp.status() == StatusCode::INTERNAL_SERVER_ERROR
            && resp.headers().contains_key(MESSAGE_TYPE)
    }
    ...
    pub(crate) fn retry(
        ...
        let Ok(resp) = result.as_ref() else {
            return Some(self.create_retry());
        };
