...
impl EncryptedClient {
        ...
        C: Crypto,
    {
        ...
        let msg = self.inner.send(&msg).await?;
...
impl HttpClient {
    ...
    async fn execute(&mut self, req: reqwest::Request) -> Result<HttpResponse, Error> {
    ...
    async fn send_with_retry(&mut self, req: reqwest::Request) -> Result<reqwest::Response, Error> {
        ...
        let mut resp = self.inner.execute(req).await.map_err(|err| {
