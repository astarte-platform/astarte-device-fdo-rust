...
pub trait Crypto {
    ...
    fn hmac(
        ...
        header: &[u8],
    ) -> impl std::future::Future<Output = Result<HMac<'static>, Error>> + Send;
    ...
    fn cose_sing(
        ...
        payload: Vec<u8>,
    ) -> impl std::future::Future<Output = Result<CoseSign1, Error>> + Send;

    fn create_nonce(&mut self) -> impl std::future::Future<Output = Result<Nonce, Error>> + Send;

    fn verify_cose_singature(sign: &CoseSign1, pub_key: &PublicKey) -> Result<(), Error> {
        let alg = sign
            .protected
            .header
            .alg
    ...
    ) -> impl std::future::Future<Output = Result<(XBKeyExchange<'static>, Self::KeyExchange), Error>>
           + Send;
